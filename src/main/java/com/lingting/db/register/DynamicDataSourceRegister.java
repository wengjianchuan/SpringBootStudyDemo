package com.lingting.db.register;
...
import com.lingting.db.config.DynamicRoutingDataSource;
...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
     ...
     * 存储我们注册的数据源
     */
    private Map<String, javax.sql.DataSource> customDataSources = new HashMap<String, javax.sql.DataSource>();
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        // 获取数据源类型
        Class<? extends javax.sql.DataSource> clazz = getDataSourceType(typeStr);
        // 绑定默认数据源参数 也就是主数据源
        javax.sql.DataSource consumerDatasource, defaultDatasource = bind(clazz, defauleDataSourceProperties);
        DynamicDataSourceContextHolder.dataSourceIds.add("master");
        log.info("注册默认数据源成功");
        // 获取其他数据源配置
        List<Map> configs = binder.bind("slave.datasource", Bindable.listOf(Map.class)).get();
	...
	private void bind(javax.sql.DataSource result, Map properties) {
     ...
     */
    @SuppressWarnings({ "unused", "rawtypes" })
	private <T extends javax.sql.DataSource> T bind(Class<T> clazz, String sourcePath) {
